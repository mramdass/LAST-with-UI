...
//	LAST (C) 2015 Munieshwar Ramdass
//	
...
namespace File_Generator {
	...
	// Returns nothing
	void create_files(const string& name, const string& dir) {
		csv_database db;
		read_csv(db, name);
		if (db.size() == 0)
			return;
		...
		int file_num(1);
		while (true) {
			ofstream ofs(dir + "/Evidence_" + to_string(file_num) + ".csv");
